...
In addition the source code implies the availability of the following which don't *seem* to be mentioned in the documentation: `first_available_file` (deprecated), `transport`, 
...
```
    ...
    - debug: {% raw %}msg="{{ item.0 }} and {{ item.1 }}{% endraw %}"
      with_together:
        - alpha
        - numbers
```
...
```
    ...
    - authorized_key: "{% raw %}user={{ item.0.name }} key='{{ lookup('file', item.1) }}'{% endraw %}"
      with_subelements:
         - users
         - authorized
```
...
```
    - name: indexed loop demo
      debug: msg="{% raw %}at array position {{ item.0 }} there is a value {{ item.1 }}{% endraw %}"
